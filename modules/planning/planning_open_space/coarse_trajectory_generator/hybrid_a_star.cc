...
double HybridAStar::TrajCost(std::shared_ptr<Node3d> current_node, std::shared_ptr<Node3d> next_node) {
    ...
    ADEBUG << "steer pos: " << traj_steer_penalty_ * std::abs(next_node->GetSteer());
...
bool HybridAStar::Plan(
        ...
        bool is_vertical,
        double kappa_ratio) {
    max_steer_angle_ = kappa_ratio * max_steer_angle_;
    max_kappa_ = std::tan(max_steer_angle_) / vehicle_param_.wheel_base();
    reed_shepp_generator_->reeds_sheep_last_straight_ = reeds_sheep_last_straight;
    reed_shepp_generator_->is_vertical_ = is_vertical;
    reed_shepp_generator_->max_kappa_ = max_kappa_;
    // clear containers
    open_set_.clear();
    close_set_.clear();
