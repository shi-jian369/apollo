...
ReedShepp::ReedShepp(const common::VehicleParam& vehicle_param, const PlannerOpenSpaceConfig& open_space_conf) :
...
ReedShepp::ReedShepp(const common::VehicleParam& vehicle_param, const WarmStartConfig& warm_start_conf) :
        vehicle_param_(vehicle_param), planner_warm_start_config_(warm_start_conf) {
    max_kappa_ = std::tan(
                         vehicle_param_.max_steer_angle() * planner_warm_start_config_.traj_kappa_contraint_ratio()
                         / vehicle_param_.steer_ratio())
            / vehicle_param_.wheel_base();
    ...
    traj_straight_length_ = planner_warm_start_config_.traj_straight_length();
}
...
bool ReedShepp::ShortestRSP(
    ...
    if (reeds_sheep_last_straight_ && !is_vertical_) {
        auto& path = all_possible_paths[optimal_path_index];
        path.segs_lengths.push_back(1.0 * max_kappa_ / path.radius);
        path.segs_lengths.push_back(-1.0 * max_kappa_ / path.radius);
        path.segs_types.push_back('S');
        path.segs_types.push_back('S');
        path.total_length += 2 * max_kappa_ / path.radius;
    }

    if (!GenerateLocalConfigurations(start_node, end_node, &(all_possible_paths[optimal_path_index]))) {
    ...
    if (std::abs(all_possible_paths[optimal_path_index].x.back() - end_node->GetX()) > 1e-3
        ...
        || common::math::NormalizeAngle(all_possible_paths[optimal_path_index].phi.back() - end_node->GetPhi())
                > 1e-3) {
        ...
        ADEBUG << "end x, y, phi are: " << end_node->GetX() << ", " << end_node->GetY() << ", " << end_node->GetPhi();
        return false;
    }
...
bool ReedShepp::SCS(const double x, const double y, const double phi, std::vector<ReedSheppPath>* all_possible_paths) {
    RSPParam SLS_param;
...
bool ReedShepp::CS(const double x, const double y, const double phi, std::vector<ReedSheppPath>* all_possible_paths) {
    RSPParam LS1_param;
...
bool ReedShepp::CSC(const double x, const double y, const double phi, std::vector<ReedSheppPath>* all_possible_paths) {
    RSPParam LSL1_param;
...
bool ReedShepp::CCC(const double x, const double y, const double phi, std::vector<ReedSheppPath>* all_possible_paths) {
    RSPParam LRL1_param;
...
bool ReedShepp::CCCC(const double x, const double y, const double phi, std::vector<ReedSheppPath>* all_possible_paths) {
...
bool ReedShepp::CCSC(const double x, const double y, const double phi, std::vector<ReedSheppPath>* all_possible_paths) {
...
void ReedShepp::LSL(const double x, const double y, const double phi, RSPParam* param) {
    std::pair<double, double> polar = common::math::Cartesian2Polar(x - std::sin(phi), y - 1.0 + std::cos(phi));
...
void ReedShepp::LS(const double x, const double y, const double phi, RSPParam* param) {
    ...
    double epsilon = 1e-6;
    if (std::fabs(phi_mod - M_PI_2) < epsilon || std::fabs(phi_mod) < epsilon || std::fabs(phi_mod + M_PI_2) < epsilon
...
void ReedShepp::LRSLR(const double x, const double y, const double phi, RSPParam* param) {
    ...
    if (rho >= 2.0) {
        ...
        if (u <= 0.0) {
            t = common::math::NormalizeAngle(atan2((4.0 - u) * xi - 2.0 * eta, -2.0 * xi + (u - 4.0) * eta));
            v = common::math::NormalizeAngle(t - phi);
...
bool ReedShepp::GenerateLocalConfigurations(
    ...
    double step_scaled = planner_warm_start_config_.step_size() * max_kappa_;
    ADEBUG << "shortest_path->total_length" << shortest_path->total_length;
    size_t point_num = static_cast<size_t>(std::floor(
            shortest_path->total_length / step_scaled * radius + static_cast<double>(shortest_path->segs_lengths.size())
            + 4));
    ...
    for (size_t i = 0; i < shortest_path->segs_types.size(); ++i) {
        ...
        ll = l - pd - d;
        index++;
        Interpolation(std::min(index, int(point_num-1)), l, m, ox, oy, ophi, radius, &px, &py, &pphi, &pgear);
    }
    ...
    for (size_t i = 0; i < px.size(); ++i) {
        shortest_path->x.push_back(
                std::cos(-start_node->GetPhi()) * px.at(i) + std::sin(-start_node->GetPhi()) * py.at(i)
                + start_node->GetX());
        shortest_path->y.push_back(
                -std::sin(-start_node->GetPhi()) * px.at(i) + std::cos(-start_node->GetPhi()) * py.at(i)
                + start_node->GetY());
