...
GridSearch::GridSearch(const WarmStartConfig& warm_start_conf) {
    xy_grid_resolution_ = warm_start_conf.grid_a_star_xy_resolution();
    node_radius_ = warm_start_conf.node_radius();
    esdf_range_ = warm_start_conf.esdf_range();
    use_esdf_ = warm_start_conf.use_esdf();
}

double GridSearch::EuclidDistance(const double x1, const double y1, const double x2, const double y2) {
...
/* Currently it is only used to add cost at the boundaries of parking spaces,
thus avoiding the generation of trajectories that press on the parking space lines.
Since the parking space information is fixed, it is only called once.*/
void GridSearch::AddSoftCost(const std::vector<std::vector<common::math::LineSegment2d>>& soft_boundary) {
    for (const auto& soft_boundary_linesegments : soft_boundary) {
        for (const common::math::LineSegment2d& linesegment : soft_boundary_linesegments) {
            int max = linesegment.length() / node_radius_;
...
void GridSearch::ExtendNode(
    ...
    // << origin_index.x() << " origin y: " << origin_index.y();
    // AINFO << "min_dist: " << min_dist;
    if (min_dist > esdf_range_ || x > max_grid_x_ || x < 0 || y > max_grid_y_ || y < 0) {
        return;
    }
    ...
    if (close_set.find(index) == close_set.end() && dp_map_.find(index) != dp_map_.end()
        ...
        for (int i = 0; i < 4; i++) {
            int new_x = x + dx_[i];
            int new_y = y + dy_[i];
            ExtendNode(new_x, new_y, origin_index, close_set);
        }
    } else {
        return;
    }
...
std::vector<std::shared_ptr<Node2d>> GridSearch::GenerateNextNodes(std::shared_ptr<Node2d> current_node) {
...
bool GridSearch::GenerateDpMap(
    ...
    if (use_esdf_) {
        AddSoftCost(soft_boundary_linesegments_vec);
    }
    ADEBUG << "explored node num is " << explored_node_num;
    return true;
}
