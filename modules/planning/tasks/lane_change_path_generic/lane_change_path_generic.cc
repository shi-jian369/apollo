...
#include "modules/planning/tasks/lane_change_path_generic/lane_change_path_generic.h"
...
bool LaneChangePathGeneric::Init(
    ...
    return Task::LoadConfig<LaneChangePathGenericConfig>(&config_);
}

apollo::common::Status LaneChangePathGeneric::Process(Frame* frame, ReferenceLineInfo* reference_line_info) {
...
bool LaneChangePathGeneric::DecidePathBounds(std::vector<PathBoundary>* boundary) {
    ...
    path_bound.DebugString("from_self_lane_l");
    ...
    path_bound.DebugString("extend_boundary_by_ADC_l");

    PathBound temp_path_bound = path_bound;
    std::string blocking_obstacle_id;
    std::vector<SLPolygon> obs_sl_polygons;
    GetSLPolygons(&obs_sl_polygons);
    ...
    path_bound.DebugString("from_static_obstacles_l");

    // 3. Remove the S-length of target lane out of the path-bound.
    if (config_.enable_forbidden_zone()) {
        GetBoundaryFromLaneChangeForbiddenZone(&path_bound);
        path_bound.DebugString("forbidden_zone_l");
    }

    GetBoundaryFromNudgeDecision(&path_bound);
    path_bound.DebugString("from_nudge_decision_l");
...
bool LaneChangePathGeneric::OptimizePath(
...
bool LaneChangePathGeneric::AssessPath(std::vector<PathData>* candidate_path_data, PathData* final_path) {
...
void LaneChangePathGeneric::UpdateLaneChangeStatus() {
...
bool LaneChangePathGeneric::IsClearToChangeLane(ReferenceLineInfo* reference_line_info) {
...
void LaneChangePathGeneric::GetLaneChangeStartPoint(
...
void LaneChangePathGeneric::GetBoundaryFromLaneChangeForbiddenZone(PathBoundary* const path_bound) {
    // Sanity checks.
...
bool LaneChangePathGeneric::GetBoundaryFromNudgeDecision(PathBoundary* const path_bound) {
    bool is_right_change_lane = reference_line_info_->Lanes().PreviousAction() == routing::ChangeLaneType::RIGHT;
    ...
    int key_points_index = is_right_change_lane ? 0 : 1;
...
void LaneChangePathGeneric::UpdateStatus(
        double timestamp,
...
bool LaneChangePathGeneric::HysteresisFilter(
...
void LaneChangePathGeneric::SetPathInfo(PathData* const path_data) {
...
bool LaneChangePathGeneric::CheckLastFrameSucceed(const apollo::planning::Frame* const last_frame) {
    if (last_frame) {
...
void LaneChangePathGeneric::GetSLPolygons(std::vector<SLPolygon>* polygons) {
    polygons->clear();
