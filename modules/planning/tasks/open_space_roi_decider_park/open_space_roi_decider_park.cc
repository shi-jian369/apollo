...
void OpenSpaceRoiDeciderPark::SetParkingSpotEndPose(const ParkingInfo &parking_info, Frame *const frame) {
    ...
    if (parking_info.parking_type == ParkingType::VERTICAL_PARKING) {
        ...
        double vertical_heading = (left_down - left_top).Angle();
        // Vec2d middle_left = (left_top + left_down) / 2.0;
        end_pt = left_top
                + Vec2d::CreateUnitVec2d(vertical_heading)
                        * (vehicle_params_.left_edge_to_center() + config_.parallel_parking_depth())
                + Vec2d::CreateUnitVec2d(parking_heading)
                        * (vehicle_params_.back_edge_to_center() + parking_depth_buffer);
    }
...
bool OpenSpaceRoiDeciderPark::GetParkingBoundary(
    ...
    if (average_l < 0) {
        ...
        // stitching the left lane boundary with right lane boundary
        {
            std::vector<Vec2d> segment_right_left_connect{right_lane_boundary.back(), left_lane_boundary.back()};
            roi_parking_boundary->push_back(segment_right_left_connect);
        }
...
bool OpenSpaceRoiDeciderPark::GetParkingSpot(Frame *const frame, ParkingInfo *parking_info) {
    ...
    if (parallel_dist > verticle_dist) {
        parking_info->parking_type = ParkingType::PARALLEL_PARKING;
        Vec2d direc_vec = parking_info->corner_points[1] - parking_info->corner_points[0];
        direc_vec /= direc_vec.Length();
        Vec2d expand_vec = config_.expand_parking_buffer() * direc_vec;
        parking_info->corner_points[1] += expand_vec;
        parking_info->corner_points[2] += expand_vec;
    } else {
        parking_info->parking_type = ParkingType::VERTICAL_PARKING;
    }
    return true;
...
bool OpenSpaceRoiDeciderPark::GetParkingOutBoundary(
    ...
    for (const auto &parking_overlap : parking_lots) {
        ...
        if (is_in_parking_spot) {
            ...
            apollo::common::math::LineSegment2d first_segment(points.at(0), points.at(1));
            apollo::common::math::LineSegment2d second_segment(points.at(1), points.at(2));
            if (first_segment.length() > second_segment.length()) {
                AINFO << "In Parallel parking space";
                auto expand_length_unit = first_segment.unit_direction() * config_.expand_parking_buffer() * 0.1;
                points.at(0) -= expand_length_unit;
                points.at(1) += expand_length_unit;
                points.at(2) += expand_length_unit;
                points.at(3) -= expand_length_unit;
            } else {
                AINFO << "In Vertical parking space";
            }
            for (size_t i = 0; i < points.size(); i++) {
